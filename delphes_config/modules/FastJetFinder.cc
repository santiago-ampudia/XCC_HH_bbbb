...
void FastJetFinder::Process()
{
  ...
  while((candidate = static_cast<Candidate *>(fItInputArray->Next())))
  {
    ...
    if (abs(momentum.Eta()) > 1.83178) continue;
  ...
  if(fExclusiveClustering)
  {
    try
    {
      ...
      {
        // exclusive njet mode
        outputList = sorted_by_pt(sequence->exclusive_jets_up_to(fNJets));   //////changed the clustering sequence from exclusive_jets to exclusive_jets_up_to
        
        ////dcut
        double rtd = sqrt(sequence->exclusive_dmerge(fNJets-1));
        //cout<<"rtd: "<<rtd<<endl;
 	      //rtd_min = 30.0;
 	      if (rtd < rtd_min) outputList.clear();
        ////dcut
